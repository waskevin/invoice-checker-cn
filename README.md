...
安装包会同时启动后台快捷键助手。默认快捷键为 `Alt+R`，可在软件顶部修改。推荐用法是在资源管理器中多选 PDF，先按 `Ctrl+C` 复制这些文件，再按 `Alt+R`，程序会从剪贴板文件列表导入、自动改名，并复制本批有效金额合计。部分电脑也支持选中 PDF 后直接按快捷键，但以 `Ctrl+C` 后再按快捷键为准。
